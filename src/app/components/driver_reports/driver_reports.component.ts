...
export class DriverReportsComponent implements OnInit {
  ...
  getWeekCommissions() {
    ...
    if (!this.weekSubscription) {
      this.expenseService.getDriverCommission(this.driver.id, this.uid, 'week').subscribe(data => {
        this.totalWeekCommissions = 0;
        this.weekSubscription = true;
        this.weekCommissions = data;
        const DaysOfWeek = [];
  ...
  getMonthCommissions() {
    this.util.openLoader();
    this.expenseService.getDriverMonthCommission(this.driver.id, this.uid).subscribe(data => {
      this.monthSubscription = true;
      this.totalMonthCommissions = 0;
      this.monthCommissions = data;
      const weeksOfMonth = [];
  ...
  showReport(type: string) {
    this.tab = type;
    if (type === 'month' && !this.monthSubscription) {
